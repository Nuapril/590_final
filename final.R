...
### Description --------------------------------------------------------------------------
#This script accomplishes data analysis as outlined by 590 Final description

#This script uses relative qPCR data. It requires a data frame in which you have a 3 technical replicates for each of the following: a control gene sample (Y4I here), and experimental sample (pgaR), 3 reference genes (alaS, rpoC, map) for each sample, and your gene of interest (igiD here). 
#The data frame should be set up with columns labeled "sample", "gene", "replicate", "E", and "Ct"
#The efficiency ("E") and cycle threshold ("Ct") values are generated by the qPCR opticon
...
# * In order to calculate geometric mean and geometric standar deviation, I had to write a couple functions
...
## used the functions above complete this task 
...
#create a new dataframe with all samples (Y4I, pgaR, phaR, phaI, igiD) and the nrq for each sample
